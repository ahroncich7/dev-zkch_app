...
**/
var kchApp = (function (jQ, win, doc) {
    "use strict";
    var kchApp = { AppInfo: { name: "ICOCrypto", package: "1.9.5", version: "1.9.5", author: "Softnio" } },
    ...
    kchApp.components = components;
    kchApp.docReady = docReady;
    kchApp.winLoad = winLoad;

    return kchApp;
}(jQuery, window, document));

window.kchApp = kchApp;

kchApp = function (kchApp, $, window, document) {
    "use strict";
    // Defined Variables
    var $win = $(window),
        $doc = $(document),
    ...
    // Return Check @v1.0
    kchApp.Win = {};
    kchApp.Win.height = $(window).height();
    kchApp.Win.width = $(window).width();

    // getStatus @v1.0
    kchApp.getStatus = {};
    kchApp.getStatus.isRTL = ($body.hasClass('has-rtl') || $body.attr('dir') === 'rtl') ? true : false;
    kchApp.getStatus.isTouch = (("ontouchstart" in document.documentElement)) ? true : false;
    kchApp.getStatus.isMobile = (navigator.userAgent.match(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Windows Phone|/i)) ? true : false;
    kchApp.getStatus.asMobile = (kchApp.Win.width < _mobBreak) ? true : false;
    ...
    kchApp.Util = {};
    // ClassInit !Util @v1.0
    kchApp.Util.classInit = function () {
        var hastouch = function () {
            if (kchApp.getStatus.isTouch === true) {
            ...
            mobileview = function () {
                if (kchApp.getStatus.asMobile === true) {
    ...
    kchApp.components.docReady.push(kchApp.Util.classInit);

    // PreLoader !Util @v1.0
    kchApp.Util.preLoader = function () {
    ...
    //     }
    // };
    // kchApp.components.docReady.push(kchApp.Util.backTop);

    // // Browser Check !Util @v1.0
    // kchApp.Util.browser = function() {
    ...
    //     }
    // };
    // kchApp.components.winLoad.push(kchApp.Util.browser);

    // HeaderSticky !Util @v1.0
    kchApp.Util.headerSticky = function () {
    ...
    kchApp.Util.progressBar = function () {
        ...
        if ($data_point.exists() && kchApp.getStatus.isRTL === true) {
            $data_point.each(function () {
    ...
    kchApp.components.docReady.push(kchApp.Util.progressBar);

    // inputAnimation @v1.0
    kchApp.Util.inputAnimate = function () {
    ...
    kchApp.components.docReady.push(kchApp.Util.inputAnimate);

    // Dropdown @v1.0
    kchApp.Util.toggler = function () {
    ...
    kchApp.components.docReady.push(kchApp.Util.toggler);

    // accordionActive @v1.0
    kchApp.Util.accordionActive = function () {
    ...
    kchApp.components.docReady.push(kchApp.Util.accordionActive);

    // scrollAnimation !Util @v1.0
    kchApp.Util.scrollAnimation = function () {
    ...
    kchApp.MainMenu = function () {
        ...
        MenuInit.navClose = function () {
            ...
            $win.on('resize', function (e) {
                if (!kchApp.getStatus.asMobile && $win.width() > _navBreak && $body.hasClass('overlay-' + _open_menu)) {
                    $body.removeClass('overlay-' + _open_menu);
                    $navbar_toggle.removeClass(_active_nav);
                }
            });
    ...
    kchApp.components.docReady.push(kchApp.MainMenu);


    // OnePageScroll @v1.0
    kchApp.OnePageScroll = function () {
    ...
    kchApp.components.docReady.push(kchApp.OnePageScroll);

    //scrollAct @v1.0
    kchApp.scrollAct = function () {
    ...
    kchApp.components.docReady.push(kchApp.scrollAct);

    //ModalFix @v1.0
    kchApp.modalFix = function () {
        var $modal = $('.modal');
    ...
    kchApp.components.docReady.push(kchApp.modalFix);
    ...
    kchApp.Plugins.countdown = function () {
        var $count = $('.countdown');
        if ($count.exists()) {
            $count.each(function () {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.countdown);


    // Carousel !Plugin @v1.1
    kchApp.Plugins.carousel = function () {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.carousel);

    // Flex Slider !Plugin @v1.9.1
    kchApp.Plugins.flexslider = function () {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.flexslider);

    // Select2 !Plugin @v1.0
    kchApp.Plugins.select2 = function () {
        var $select = $(".select");
        if ($select.exists()) {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.select2);

    // Validator !Plugin @v1.0
    kchApp.Plugins.validform = function () {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.validform);

    // Form Validation !Plugin @v1.0
    kchApp.Plugins.submitform = function () {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.submitform);

    // Parallax !Plugin @v1.0
    kchApp.Plugins.parallax = function () {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.parallax);

    // Popup !Plugin @v1.0
    kchApp.Plugins.popup = function () {
    ...
    kchApp.components.docReady.push(kchApp.Plugins.popup);

    // particlesJS !Plugin @v1.0
    kchApp.Plugins.particles = function () {
